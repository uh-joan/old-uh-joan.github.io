		...
		$('.styled_list').each(function(){
			...
			cur.children('li').each(function(index, element){
				...
				if(type == 'process_box'){
					...
					var num_block = $('<div class="num" style="background-color: ' + num_background + '"><span>' + num + '</span></div>');
				} else if(type == 'large_numbers'){
					var num = index + 1 + '.';
					var num_color = colors[index%colors.length];
					var num_block = $('<div class="num" style="color: ' + num_color + '"><span>' + num + '</span></div>');
				}
